...
module.exports = sender => async (body, { messageId, label = '', contentType, contentEncoding, applicationProperties } = {}) => { // eslint-disable-line object-curly-newline
	const message = {
	...
	while (!isMessageSent && attempts < 3) {
		try {
			await sender.sendMessages(message); // eslint-disable-line no-await-in-loop
			isMessageSent = true;
		} catch (err) {
			lastError = err;
			attempts++;
